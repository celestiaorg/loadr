...
use loadr_webui::{AuthConfig, EngineLauncher, LocalBackend, WebUi, WebUiConfig, WebUiHandle};

// ---------------------------------------------------------------------------
// Harness
// ---------------------------------------------------------------------------

/// Mock gRPC protocol: status OK, 1-3ms latency, realistic timings.
#[derive(Default)]
struct MockGrpc {
    counter: AtomicU64,
}
...
impl ProtocolHandler for MockGrpc {
    fn name(&self) -> &str {
        "grpc"
    }
...
fn mock_launcher() -> EngineLauncher {
    Arc::new(|plan, base_dir, run_id| {
        ...
        protocols.register(Arc::new(MockGrpc::default()));
...
async fn run_lifecycle_and_summary() {
    ...
    let grpc_reqs = metrics
        .iter()
        .find(|m| m["metric"] == "grpc_reqs")
        .expect("grpc_reqs metric");
    assert_eq!(grpc_reqs["agg"]["sum"], 20.0);
