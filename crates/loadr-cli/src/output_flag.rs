...
/// Parse `prometheus=listen_addr[,final_scrape_grace=dur]`.
pub fn parse_output_flag(spec: &str) -> Result<OutputConfig, String> {
    ...
    match kind {
        ...
        other => Err(format!(
            "unknown output kind `{other}` (expected prometheus)"
        )),
    }
}
...
mod tests {
    ...
    fn parses_kinds() {
        ...
        let OutputConfig::Prometheus {
            listen,
            final_scrape_grace,
            ..
        } = parse_output_flag("prometheus=127.0.0.1:9091,final_scrape_grace=10s").unwrap();
