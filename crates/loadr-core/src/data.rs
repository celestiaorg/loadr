...
impl Default for DataFeeds {
    fn default() -> Self {
        Self {
            feeds: HashMap::new(),
            has_on_demand: false,
            run_id: Arc::from("local"),
...
impl DataFeeds {
    ...
    /// Set the distributed identity attached to every feeder invocation.
        ...
        let feed = match feed {
            Feed::Plugin { plugin, blocking } => {
                ...
                let ctx = PluginRowCtx {
                    ...
                    partition_index: self.partition.0,
                    partition_count: self.partition.1,
                    source,
                    vu: id.vu,
