...
pub fn execute(args: ControllerArgs) -> anyhow::Result<i32> {
    ...
    runtime.block_on(async move {
        ...
        let mut auth = loadr_webui::AuthConfig::default();
        ...
        let ui = loadr_webui::WebUi::serve(loadr_webui::WebUiConfig {
            bind: args.ui_bind,
            auth,
        ...
        if let Some(listen) = &args.prometheus_listen {
            let mut output = loadr_prometheus::PrometheusOutput::new(
                Some(listen.clone()),
                None,
...
impl ControllerBackend {
    ...
    fn current_runs(&self) -> Vec<loadr_webui::RunInfo> {
        self.controller
            .runs()
            .into_iter()
            .map(|run| {
    ...
    fn sync_history(&self, runs: &[loadr_webui::RunInfo]) {
...
impl loadr_webui::UiBackend for ControllerBackend {
    async fn start_test(
        &self,
        name: Option<String>,
        yaml: String,
    ...
    fn run_control_state(&self, run_id: &str) -> loadr_webui::RunControlView {
        self.controller
            .run_operational_info(run_id)
            .map(|info| loadr_webui::RunControlView {
                ...
                if path.extension().and_then(|e| e.to_str()) == Some("yaml") {
                    if let Ok(yaml) = std::fs::read_to_string(&path) {
                        let updated_ms = entry
                            ...
                            .unwrap_or(0);
                        out.push(loadr_webui::StoredTest {
                            name: path
                                .file_stem()
