                ...
                let action = match cur.as_ref() {
                    Some(RunPhase::Preparing { run_id, cancel, .. }) if *run_id == c.run_id => {
                        if stopish {
                            ...
                            Action::Ignore("control ignored while preparing")
                        }
                    }
                    ...
                    _ => Action::Ignore("control for unknown run ignored"),
                };
                match action {
                    Action::CancelPrep => {
                    ...
                    Action::Disarm => {
                        ...
                        if let Some(RunPhase::Armed(run)) = cur.take() {
                            drop(cur);
                    ...
                    Action::Ignore(why) => {
                        tracing::debug!(run_id = %c.run_id, action = %c.action, why);
                        return;
                    }
                }
            };
