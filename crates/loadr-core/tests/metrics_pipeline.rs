...
async fn no_output_run_uses_shards_and_reports_exact_totals() {
    ...
    let loaded = grpc_fixture::load(PLAN_YAML);
    ...
    assert_eq!(
        grpc_reqs.agg.sum, EXPECTED_ITERATIONS,
        "exact grpc_reqs total"
    );
...
async fn sample_consuming_output_keeps_bus_path() {
    ...
    let grpc_reqs = result
        ...
        .find(|m| m.metric == "grpc_reqs")
        .expect("grpc_reqs metric");
    assert_eq!(grpc_reqs.agg.sum, EXPECTED_ITERATIONS);
    let iterations = result
        .summary
        .metrics
        .iter()
...
async fn setup_failure_returns_error_not_hang_shard_mode() {
    ...
    let loaded = grpc_fixture::load(NEVER_RUN_PLAN);
    let engine = Engine::new(
        loaded.plan,
        std::path::PathBuf::from("."),
        EngineOptions {
